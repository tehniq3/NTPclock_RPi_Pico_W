 ...
 * v.2.18a - activated PWM signal for all cases
 * v.2.18b - replaced main WHILE with IF (to work without external watchdog)
 * v.2.19 - in running mode, retry to reconnect to wifi after disconnet + NTP check more rarely
...
const unsigned long updateDelay = 120000;         // update time every 15 min
const unsigned long retryDelay  = 60000;         // retry 5 min later if time query failed
...
unsigned long tpfortare = 2*3600000; // x hours without wether info 
