 ...
 * v.2.9b - changed ESP8266WiFi with WiFi libraries
...
String result;
String APIKEY = "ca55295c4a1dce2688e0751d4b9a68de";  // "ca55295c4a4b9a681dce2688e0751dde"; // https://home.openweathermap.org/api_keys                            
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED && millis() - lastUpdatedTime >= updateDelay) 
  {
    ...
    if (updated) {
      ...
      //matrix.setPixelColor(0, 150, 150, 0);
      //matrix.show();
      wificon = 1;
  ...
  //matrix.setPixelColor(0, 150, 0, 0);
  //matrix.show();
 // digitalWrite(initial, HIGH);  // force RESET in HIGH (not reset)
 // WiFi.disconnect();
 //  delay(1000);
 // WiFi.reconnect();
 // WiFi.begin(ssid, password);
 ...
 //   //Serial.print(".");
  //}
 // digitalWrite(piviem, millis()%2);
 // digitalWrite(initial, LOW);  // force RESET in HIGH (not reset)
    }
  }
...
if (wificon == 0)
 matrix.setPixelColor(0, 150, 0, 0); 
// 2 - weather data received (green), 1 - weather connected (blue), 0 = weather not-connected (red) 
if (vremecon == 2)
